...
async function load(){let data=[],error=null;try{for(let from=0;;from+=500){const r=await client.from('products').select('*').order('created_at',{ascending:false}).range(from,from+499);if(r.error){error=r.error;break}if(r.data?.length)data.push(...r.data);if(!r.data||r.data.length<500)break}}catch(e){error=e}if(error){$('list').innerHTML='<div class="notice">'+esc(error.message||error)+'</div>';return}products=data||[];renderProductList()}
function renderProductList(){const list=$('list'),count=$('productFilterCount');if(!list)return;const term=($('productSearch')?.value||'').trim().toLocaleLowerCase('pt-BR');const category=$('productCategoryFilter')?.value||'';const filtered=products.filter(p=>{const name=String(p.name||'').toLocaleLowerCase('pt-BR');const code=String(p.product_code||'');const sector=String(p.sector||'');return (!term||name.includes(term)||code.includes(term))&&(!category||sector===category)});if(count)count.textContent=term||category?`${filtered.length} produto(s) encontrado(s)`:`${products.length} produto(s) cadastrado(s)`;list.innerHTML=filtered.length?filtered.map(p=>`<div class="product-admin-row"><img class="thumb" src="${esc(p.image_url||'')}" alt=""><div class="grow"><b>${esc(p.name)} ${p.is_featured?'<span class=\"featured-admin-badge\">⭐ Destaque</span>':''}</b><small>${esc(p.sector)} • ${p.unit==='kg'?'Kg':'Unidade'} • ${money(p.price)}${p.note?` • 📝 ${esc(p.note)}`:''} ${p.promo_price!=null?'• Promo '+money(p.promo_price):''} ${p.wholesale_price!=null?'• Atacado '+money(p.wholesale_price)+' / '+p.wholesale_qty+' '+(p.unit==='kg'?'kg':'un.') :''} • ${p.in_stock===false?'Sem estoque':'Em estoque'}</small></div><button class="outline-btn" onclick="edit('${p.id}')">Editar</button><button class="danger" onclick="removeProduct('${p.id}')">Excluir</button></div>`).join(''):'<div class="empty-state">Nenhum produto encontrado com esses filtros.</div>'}
function clearProductFilters(){if($('productSearch'))$('productSearch').value='';if($('productCategoryFilter'))$('productCategoryFilter').value='';renderProductList()}
