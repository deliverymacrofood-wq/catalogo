...
function orderCard(o){
  ...
  return `<article class="order-card" data-status="${esc(o.status)}"><div class="order-head"><div><span class="order-number">Pedido #${o.order_number}</span><small>${new Date(o.created_at).toLocaleString('pt-BR')}</small></div><span class="status ${statusClass[o.status]||''}">${statusLabel[o.status]||o.status}</span></div><div class="order-body"><div class="customer-box"><b>Cliente</b><strong>${esc(o.customer_name)}</strong><span class="customer-phone">📱 <b>Contato:</b> ${esc(phoneLabel)}</span>${o.customer_email?`<span>✉ ${esc(o.customer_email)}</span>`:''}${o.document_number?`<span>🪪 ${o.document_type==='cnpj'?'CNPJ':'CPF'}: ${esc(o.document_number)}</span>`:''}${o.zipcode?`<span>📍 CEP: ${esc(o.zipcode)}</span>`:''}${o.sales_customer?`<span>💬 Cadastro no WhatsApp de vendas</span>`:''}${o.note?`<div class="note">📝 ${esc(o.note)}</div>`:''}</div><div class="order-items">${items.map(i=>`<div><span>${esc(i.name)} × ${i.qty}</span><strong>${money(i.subtotal)}</strong></div>`).join('')}<div class="order-total"><span>Total</span><strong>${money(o.total)}</strong></div></div></div><div class="order-actions">${waButton}${o.status==='received'?`<button class="primary" onclick="finalizeAndNotify('${o.id}','${o.customer_phone}','${esc(o.customer_name)}',${o.order_number})">✅ Finalizar e avisar no WhatsApp</button>`:''}${o.status==='ready_payment'?`<button class="primary" onclick="markPaid('${o.id}')">💰 Marcar como pago</button><button class="whatsapp-btn" onclick="contactCustomer('${o.customer_phone}','${esc(o.customer_name)}','ready_payment',${o.order_number})">💬 Avisar para pagar</button>`:''}${o.status==='paid'?`<button class="primary" onclick="markCompleted('${o.id}')">📦 Finalizar pedido</button><button class="whatsapp-btn" onclick="contactCustomer('${o.customer_phone}','${esc(o.customer_name)}','paid',${o.order_number})">💬 Entrar em contato pelo WhatsApp</button>`:''}${['received','ready_payment','paid'].includes(o.status)?`<button class="order-chat-btn" type="button" onclick="openOrderChat('${o.id}',${o.order_number},'${esc(o.customer_name||'Cliente')}',true)">💬 Chat do pedido</button><button class="order-edit-btn" type="button" onclick="openEditOrder('${o.id}')">✏ Alterar pedido</button>`:''}${o.status==='completed'?`<button class="whatsapp-btn" onclick="contactCustomer('${o.customer_phone}','${esc(o.customer_name)}','completed',${o.order_number})">💬 Contatar cliente</button>`:''}<button class="danger" onclick="cancelOrder('${o.id}')">Cancelar</button></div></article>`
}
...
async function openEditOrder(orderId){
  ...
  if(!['received','ready_payment','paid'].includes(order.status))return alert('Este pedido já não está em separação e não pode mais ser alterado.');
...
function renderEditOrderRows(){
  ...
  el.innerHTML=state.rows.length?state.rows.map((r,i)=>{const p=map.get(String(r.product_id));if(!p)return '';return `<div class="order-edit-row"><img src="${esc(p.image_url||'')}" alt=""><div class="grow"><b>${esc(p.name)}</b><small>${money(p.price)} / ${p.unit==='kg'?'kg':'un.'}</small></div><label>Quantidade<input type="number" min="1" max="9999" step="1" value="${Number(r.qty)||1}" onchange="changeEditOrderQty(${i},this.value)"></label><button class="danger" type="button" onclick="removeEditOrderProduct(${i})">Remover</button></div>`}).join(''):'<div class="empty-state">Nenhum produto no pedido.</div>';
...
function changeEditOrderQty(i,value){const state=window.editingOrderData;if(!state)return;const q=Math.floor(Number(value));if(!Number.isInteger(q)||q<1)return renderEditOrderRows();state.rows[i].qty=Math.min(q,9999);renderEditOrderRows()}
...
async function saveEditedOrder(orderId){
  ...
  const clean=state.rows.map(r=>({product_id:r.product_id,qty:Math.floor(Number(r.qty))}));
  if(clean.some(r=>!r.product_id||!Number.isInteger(r.qty)||r.qty<1||r.qty>9999))return alert('Verifique as quantidades dos produtos.');
...
async function supportTab(){
  ...
  const {data:states}=await client.from('support_conversations').select('user_id,status,resolved_at').in('user_id',ids.length?ids:['00000000-0000-0000-0000-000000000000']);
...
function activateSupportConversation(c){
  ...
  window.selectedSupportUser=c.uid;$('supportChatTitle').textContent=c.p.nickname||'Cliente';$('supportChatEmail').textContent=c.p.email||'';renderAdminChat(c.uid);$('adminChatForm').style.display='flex';$('adminChatForm').onsubmit=sendAdminSupport;updateAdminSupportState(c);
}
function openSupportConversation(uid,btn){
  const p=window.supportProfiles.get(uid)||{};const st=window.supportStates?.get(uid)||{status:'open'};activateSupportConversation({uid,p,status:st.status,resolved_at:st.resolved_at});
}
...
async function sendAdminSupport(e){e.preventDefault();const text=$('adminChatText').value.trim(),uid=window.selectedSupportUser;if(!text||!uid)return;const {error}=await client.from('support_conversations').upsert({user_id:uid,status:'open',resolved_at:null,updated_at:new Date().toISOString()},{onConflict:'user_id'});if(error)return alert(error.message);const {error:sendError}=await client.from('support_messages').insert({user_id:uid,sender_role:'admin',message:text});if(sendError)return alert('Não foi possível enviar: '+sendError.message);$('adminChatText').value='';const {data}=await client.from('support_messages').select('id,user_id,sender_role,message,created_at').order('created_at',{ascending:false});window.supportRows=data||[];if(window.supportStates)window.supportStates.set(uid,{user_id:uid,status:'open'});renderAdminChat(uid);updateAdminSupportState({uid,status:'open'});}
async function resolveAdminSupport(uid){if(!confirm('Marcar esta conversa como problema resolvido?'))return;const {error}=await client.from('support_conversations').upsert({user_id:uid,status:'resolved',resolved_at:new Date().toISOString(),updated_at:new Date().toISOString()},{onConflict:'user_id'});if(error)return alert('Não foi possível finalizar: '+error.message);window.supportStates.set(uid,{user_id:uid,status:'resolved'});updateAdminSupportState({uid,status:'resolved'});const btn=[...document.querySelectorAll('.support-conv')].find(x=>x.getAttribute('onclick')?.includes(uid));if(btn&&!btn.innerHTML.includes('Resolvido'))btn.querySelector('b').insertAdjacentHTML('beforeend',' <span class="resolved-chip">✓ Resolvido</span>');}
async function reopenAdminSupport(uid){const {error}=await client.from('support_conversations').upsert({user_id:uid,status:'open',resolved_at:null,updated_at:new Date().toISOString()},{onConflict:'user_id'});if(error)return alert(error.message);window.supportStates.set(uid,{user_id:uid,status:'open'});updateAdminSupportState({uid,status:'open'});}
