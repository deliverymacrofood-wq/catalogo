const client=supabase.createClient(window.SUPABASE_URL,window.SUPABASE_ANON_KEY);const $=id=>document.getElementById(id);let userId=null;let timer=null;let conversationResolved=false;let supportAdminName='MacroFood';
...
async function loadAdminIdentity(){
  try{const {data,error}=await client.rpc('get_support_admin_name',{p_user_id:userId});if(!error&&data)supportAdminName=String(data);const el=$('supportAdminName');if(el)el.textContent=supportAdminName}catch(e){console.warn('Não foi possível carregar o nome do administrador:',e)}
}
async function loadConversation(){
  const {data,error}=await client.from('support_conversations').select('status,resolved_at,admin_id').eq('user_id',userId).maybeSingle();
...
async function loadMessages(){
  ...
  el.innerHTML=rows.length?rows.map(m=>`<div class="support-msg ${m.sender_role==='admin'?'admin':'user'}">${esc(m.message).replace(/\n/g,'<br>')}<small>${m.sender_role==='admin'?supportAdminName:'Você'} • ${new Date(m.created_at).toLocaleString('pt-BR')}</small></div>`).join(''):'<div class="support-empty">Olá! 👋 Envie sua primeira mensagem para falar com a MacroFood.</div>';
