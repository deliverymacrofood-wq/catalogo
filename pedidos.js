...
function orderCard(o){
  ...
  return `<article class="my-order-card"><div class="my-order-head"><div><strong>Pedido #${o.order_number}</strong><small>${new Date(o.created_at).toLocaleString('pt-BR')}</small></div><span class="status ${classes[o.status]||''}">${labels[o.status]||o.status}</span></div><div class="my-order-body"><div class="my-order-items">${items.map(i=>`<div class="my-order-item">${i.image_url?`<img src="${esc(i.image_url)}" alt="">`:''}<span>${esc(i.name)} × ${i.qty}</span><b>${money(i.subtotal)}</b></div>`).join('')}<div class="my-order-total"><span>Total</span><b>${money(o.total)}</b></div></div><div class="my-order-info"><p><b>Status atual</b><br>${statusMessage(o.status)}</p>${o.note?`<p><b>Observação</b><br>${esc(o.note)}</p>`:''}</div></div><div class="my-order-actions">${['received','ready_payment','paid'].includes(o.status)?`<button class="order-chat-btn" type="button" onclick="openOrderChat('${o.id}',${o.order_number},'${esc(o.customer_name||'')}',false)">💬 Falar sobre este pedido</button>`:''}${canCancel?`<button class="danger" onclick="cancelMyOrder('${o.id}')">✕ Cancelar pedido</button>`:''}${o.status==='ready_payment'?'<span class="order-hint">A MacroFood irá chamar você pelo WhatsApp para concluir o pagamento.</span>':''}${o.status==='paid'?'<span class="order-hint">Pagamento recebido. A equipe está preparando seu pedido.</span>':''}${o.status==='completed'?'<span class="order-hint">Pedido concluído. Obrigado pela preferência!</span>':''}${o.status==='cancelled'?'<span class="order-hint">Este pedido foi cancelado.</span>':''}</div></article>`;
}
